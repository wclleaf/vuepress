...
给`left`元素加上`float: left`后随即形成BFC，`content`元素高度塌陷，高度由`right`元素撑起
<div align=center>
...
如果将`right`元素加上`float: right;`，`content`所有元素都是float，高度塌陷为0。
<div align=center>
...
给`left`元素加上`float: left`后随即形成BFC，`content`元素高度塌陷，高度由`right`元素撑起，但同时也影响力`other`元素的内容。
<div align=center>
...
给`content`元素加上任一BFC标志形成BFC，随即`content`就不会影响`other`元素
<div align=center>
