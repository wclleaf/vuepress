    ...
    <div class="player">
      <div class="player__top">
        ...
        <div class="player-controls">
          ...
          <!-- <div class="player-controls__item" @click="showList = true">
            <svg class="icon">
              <use xlink:href="#icon-next"></use>
            </svg>
          </div> -->
      ...
      <div class="progress" ref="progress">
        <div class="progress__top">
          <div class="album-info" v-if="currentTrack">
            <div class="album-info__name">{{ currentTrack.name }}<span class="album-info__track">{{ currentTrack.artist }}</span></div>
          </div>
      ...
      <div v-cloak></div>
      <div class="player-list" v-show="showList">
        <div v-for="(track, idx) in tracks" :key="idx" class="player-list__item">
...
export default {
  ...
  methods: {
    ...
    nextTrack() {
      ...
      // if (this.currentTrackIndex -- this.)
      this.isShowCover = false;
    ...
    getRandMusic() {
      fetch('https://api.uomg.com/api/rand.music?mid=2553263397&format=json').then(res => {
        if (res.status == 200) {
          return res.json()
        }
      }).then(json => {
        // const id = json.data.url.split('?')[1].split('&')[0].split('=')[1]
        // fetch(`https://dataiqs.com/api/netease/music/?id=${id}&type=songid`).then(res => res.json()).then(j => { 
          this.tracks.push({
            name: json.data.name,
            artist: json.data.artistsname,
            cover: json.data.picurl,
            // source: j.song_url,
            source: json.data.url + '.mp3',
            favorited: false,
            // id: id
          })  
    ...
    initMusicBox() {
      ...
      if (music && JSON.parse(music).length) {
        ...
        // fetch('https://music.163.com/api/playlist/detail?id=2553263397')
        //   .then(res => res.json())
        //   .then(json => {
        //     console.log(json)
            const json = tempList
            ...
            if (json.result && json.result.tracks) {
              this.tracks = json.result.tracks.map(t => {
                return {
                  id: t.id,
                  name: t.name,
                  artist: t.artists && t.artists.length ? t.artists[0].name : '',
                  cover: t.artists && t.artists.length ? t.artists[0].picUrl : t.album.picUrl,
                  source: `https://music.163.com/song/media/outer/url?id=${t.id}.mp3`,
                  favorited: false
                }
              })
                this.currentTrack = this.tracks[0]
  ...
  created() {
    ...
    this.audio.src = null//this.currentTrack.source;
...
.player-cover__item:before {
  ...
  width: 100%;
  height: 100%;
  box-shadow: 0px 4px 10px 0px rgba(76, 70, 124, 0.5);
  display: block;
  z-index: 1;
  position: absolute;
  top: 0;
